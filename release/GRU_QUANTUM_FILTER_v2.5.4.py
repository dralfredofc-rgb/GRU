#!/usr/bin/env python3
"""
GRU Quantum Filter v2.5.4 - Heisenberg-Fourier Chunked Architecture
====================================================================
CORRECCIONES v2.5.4:
- Detección automática de aliasing en detect_time_delay:
  si f_spine > Nyquist, busca en f_alias = fs - (f_spine % fs)
  y recupera el retardo real como 1/(fs - f_peak)
- Ventana de búsqueda ajustada a [f_alias*0.5, min(f_alias*2, Nyquist)]

CORRECCIONES v2.5.2:
- Padding del último segmento para mantener shape consistente (18001 bins)
- Fix DC singularity en generate_noise (psd[0]=inf → NaN en cascada)
- was_padded capturado antes de reasignar n y segment
- Manejo de NaN en filtros y espectros

Basado en: GRU v2.5.2 (falló detección por aliasing no manejado)
"""
...
class GRUConfig:
    ...
    N_CORES = 6
    CHUNK_OUTPUT = True
    OUTPUT_JSON = "GRU_QF_LISA_1yr_v254.json"
    LOG_FILE = "gru_v254.log"
...
class HeisenbergFourierProcessor:
    def __init__(self, config):
        ...
        log(f"Heisenberg-Fourier v2.5.4 inicializado", config)
    ...
    def detect_time_delay(self, csd_result, target_delay=128.35):
        """Detecta retardo temporal con corrección automática de aliasing."""
        ...
        fs = self.config.FS
        f_nyquist = fs / 2
        f_spine = 1 / target_delay  # frecuencia física del retardo

        # Aliasing: si f_spine > Nyquist, doblar al rango [0, Nyquist]
        aliased = f_spine > f_nyquist
        if aliased:
            f_folded = f_spine % fs
            f_search = fs - f_folded if f_folded > f_nyquist else f_folded
        else:
            f_search = f_spine

        mask = (freqs > f_search * 0.5) & (freqs < min(f_search * 2, f_nyquist))
        ...
        # Recuperar retardo real: si hubo aliasing, f_real = fs - f_peak
        f_real = (fs - peak_freq) if aliased else peak_freq
        estimated_delay = 1 / f_real if f_real > 0 else float('inf')
...
class GRUQuantumFilterPipeline:
    ...
    def run(self, inject_gru=True, amplitude=1e-15, test_null=False):
        config = self.config
        log("="*60, config)
        log("GRU Quantum Filter v2.5.4 - Heisenberg-Fourier", config)
        log("CORRECCIONES: Aliasing detector, NaN fix, padding fix", config)
        log("="*60, config)
